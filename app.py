...
def get_db_cfg():
    if "db" not in st.secrets:
        raise ValueError("Missing [db] in Streamlit Secrets (Manage app → Secrets).")
    cfg = st.secrets["db"]
    required = ["host", "port", "dbname", "user", "password", "sslmode"]
    missing = [k for k in required if k not in cfg]
    if missing:
        raise ValueError(f"Missing db keys in secrets: {missing}")
    return cfg
...
# ---------------------------
# CACHING LAYER (stops freezing)
# ---------------------------
# Keep TTL short so app feels live but doesn't hammer DB.
@st.cache_data(ttl=3)
...
def add_item(name: str, unit: str, par_level: float, price_nzd: float, active: bool = True):
    ...
    item_id = fetch_df("SELECT id FROM items WHERE name=%s", (name,)).iloc[0]["id"]
    ensure_stock_rows_for_item(int(item_id))
    clear_caches()

def get_item_id_by_name(name: str):
    df = fetch_df("SELECT id FROM items WHERE name=%s AND active=TRUE", (name,))
    if df.empty:
        return None
...
def fulfill_order(order_id: int):
    ...
    ord_df = fetch_df("SELECT status FROM orders WHERE id=%s", (int(order_id),))
    if ord_df.empty:
        raise ValueError("Order not found.")
    if ord_df.iloc[0]["status"] != ORDER_STATUS_PENDING:
    ...
    for _, line in lines.iterrows():
        ...
        if prep_map.get(item_id, 0.0) < qty:
            raise ValueError(f"Not enough Prep stock for {line['name']}")
    ...
    for _, r in df.iterrows():
        ...
        if pd.isna(rid):
            ...
            ON CONFLICT (sku) DO UPDATE SET
              name=EXCLUDED.name, price=EXCLUDED.price, active=EXCLUDED.active, sort_order=EXCLUDED.sort_order
...
def upsert_menu_recipe(menu_id: int, df: pd.DataFrame):
    # wipe + reinsert for that menu_id
...
def record_pos_sale(menu_row: pd.Series, qty: float, payment_method: str, note: str = ""):
    ...
    for item_id, per_unit in recipe.items():
        ...
        adjust_stock(item_id, LOC_TRUCK, -total_used, f"POS sale: {name} x{qty}", ref_type="sale", ref_id=sale_id)
...
def connection_panel():
    with st.expander("🔧 DB connection", expanded=False):
        try:
            cfg = get_db_cfg()
            st.write("Host:", cfg.get("host"))
            st.write("Port:", cfg.get("port"))
            st.write("DB:", cfg.get("dbname"))
            st.success("Secrets loaded ✅")
            # lightweight ping
            df = fetch_df("SELECT 1 AS ok;")
            if not df.empty and int(df.iloc[0]["ok"]) == 1:
...
# POS
if section("POS") and (mobile_mode and page == "POS" or not mobile_mode):
...
# Event Mode
if section("Event Mode") and (mobile_mode and page == "Event Mode" or not mobile_mode):
...
# Orders
if section("Orders") and (mobile_mode and page == "Orders" or not mobile_mode):
    container = st.container() if mobile_mode else tabs[PAGES.index("Orders")]
    with container:
        st.subheader("Truck → Prep Kitchen Orders")
            ...
            if c3.button("Add", type="primary"):
                if pick_qty <= 0:
                    ...
                    for line in st.session_state["order_lines"]:
                        if line["Item"] == pick_item:
                            line["Qty"] += float(pick_qty)
                            found = True
                            break
                    if not found:
                        st.session_state["order_lines"].append({"Item": pick_item, "Qty": float(pick_qty)})
                    st.success("Added.")
                    st.rerun()
...
# Dashboard
if section("Dashboard") and (mobile_mode and page == "Dashboard" or not mobile_mode):
...
# Adjust Stock
if section("Adjust Stock") and (mobile_mode and page == "Adjust Stock" or not mobile_mode):
    ...
    with container:
        ...
        if items.empty:
            ...
            if st.button("Apply adjustment", type="primary"):
                try:
                    adjust_stock(int(item_map[item_name]), location, float(delta), reason=reason.strip() or "Manual adjustment")
...
# Menu Admin
if section("Menu Admin") and (mobile_mode and page == "Menu Admin" or not mobile_mode):
...
# Items
if section("Items") and (mobile_mode and page == "Items" or not mobile_mode):
    ...
    with container:
        ...
        with st.expander("Add / Update item", expanded=True):
            ...
            if st.button("Save item", type="primary"):
                try:
                    add_item(name=name, unit=unit.strip() or "unit", par_level=float(par), price_nzd=float(price), active=True)
                    st.success("Saved.")
                    st.rerun()
                except Exception as e:
                    st.error(str(e))
...
# Movements
if section("Movements") and (mobile_mode and page == "Movements" or not mobile_mode):
